...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                successors_corners = list(seen_corners)
                next_node = (nextx, nexty)
                if next_node in self.corners:
                    if next_node not in successors_corners:
                        successors_corners.append(next_node)
                successor = ([next_node, successors_corners], action, 1)
...
def cornersHeuristic(state, problem):
    ...
    while corners_left:
        cost, corner = min([(util.manhattanDistance(point, corner), corner) for corner in corners_left])
